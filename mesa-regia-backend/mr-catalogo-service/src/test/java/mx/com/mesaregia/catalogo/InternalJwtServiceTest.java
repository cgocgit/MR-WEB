...
import mx.com.mesaregia.catalogo.security.internal.InternalJwtService;
...
class InternalJwtServiceTest {
    ...
    void emiteYValidaIdentidadInternaLocal() {
        var env = new MockEnvironment().withProperty("spring.application.name", "mr-catalogo-service");
        var jwt = new InternalJwtService(new ObjectMapper(), env);
        String token = jwt.issue();
        assertEquals("mr-catalogo-service", jwt.verifyIfInternal(token, Set.of("mr-catalogo-service")).orElseThrow());
    }
