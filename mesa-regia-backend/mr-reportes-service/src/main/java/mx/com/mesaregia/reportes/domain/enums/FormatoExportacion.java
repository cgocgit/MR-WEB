...
public enum FormatoExportacion {
    ...
    public static FormatoExportacion from(String value) {
        if (value == null || value.isBlank()) throw new ReporteValidationException("El formato es obligatorio: PDF, XLSX o CSV");
        try { return valueOf(value.trim().toUpperCase(Locale.ROOT)); }
        catch (IllegalArgumentException ex) { throw new ReporteValidationException("Formato no permitido: " + value); }
    }
}
