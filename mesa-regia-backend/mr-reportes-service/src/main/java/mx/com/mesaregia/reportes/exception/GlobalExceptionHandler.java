...
public class GlobalExceptionHandler {
  ...
  ResponseEntity<ApiError> handleBadRequest(Exception ex, HttpServletRequest request) {
    return response(HttpStatus.BAD_REQUEST, ex.getMessage() == null ? "Solicitud inválida" : ex.getMessage(), request,
        List.of());
  }
  ...
  ResponseEntity<ApiError> handleExport(ReporteExportException ex, HttpServletRequest request) {
    return response(HttpStatus.INTERNAL_SERVER_ERROR, "No fue posible generar el archivo solicitado", request,
        List.of());
  }
