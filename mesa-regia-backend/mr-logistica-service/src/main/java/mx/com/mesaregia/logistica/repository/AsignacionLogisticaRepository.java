...
import mx.com.mesaregia.logistica.domain.entity.AsignacionLogistica;
...
public interface AsignacionLogisticaRepository extends JpaRepository<AsignacionLogistica, Long> {
  List<AsignacionLogistica> findAllByProgramacionIdOrderByOrdenParadaAsc(Long id);

  Optional<AsignacionLogistica> findByProgramacionIdAndIdOrdenExterno(Long idProgramacion, Long idOrden);

  boolean existsByProgramacionIdAndOrdenParadaAndEstadoNot(Long id, Integer orden, EstadoAsignacion estado);
  ...
  @Query("""
      ...
      """)
  List<AsignacionLogistica> misOperaciones(@Param("idUsuario") Long idUsuario, @Param("perfil") String perfil,
      @Param("cancelada") EstadoAsignacion cancelada);
