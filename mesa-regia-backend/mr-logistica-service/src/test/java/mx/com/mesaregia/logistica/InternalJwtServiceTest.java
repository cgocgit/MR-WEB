...
import mx.com.mesaregia.logistica.security.internal.InternalJwtService;
...
class InternalJwtServiceTest {
    ...
    void emiteYValidaIdentidadInternaLocal() {
        var env = new MockEnvironment().withProperty("spring.application.name", "mr-logistica-service");
        var jwt = new InternalJwtService(new ObjectMapper(), env);
        String token = jwt.issue();
        assertEquals("mr-logistica-service", jwt.verifyIfInternal(token, Set.of("mr-logistica-service")).orElseThrow());
    }
