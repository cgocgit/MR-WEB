...
public class InternalAuditServiceImpl implements InternalAuditService {
  ...
  @Override
  @Transactional
  public void registrar(InternalAuditEventRequest r) {
    var e = new EventoAuditoria();
    e.setModulo(r.modulo());
    e.setAccion(r.accion());
    e.setTipoRecurso(r.tipoRecurso());
    e.setIdentificadorRecurso(r.identificadorRecurso());
    e.setResultado(ResultadoAuditoria.EXITOSO);
    e.setMotivo(r.motivo());
    e.setDetalle(r.detalle());
    e.setIdCorrelacion(r.correlationId());
    repo.save(e);
  }
}
