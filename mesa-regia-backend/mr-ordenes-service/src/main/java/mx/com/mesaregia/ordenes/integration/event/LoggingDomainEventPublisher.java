...
public class LoggingDomainEventPublisher implements DomainEventPublisher {
  ...
  public LoggingDomainEventPublisher(IntegrationOutboxRepository r, ObjectMapper j) {
    repo = r;
    json = j;
  }
  ...
  @Override
  @Transactional
  public void publish(OrdenEstadoCambiadoEvent event) {
    try {
      var e = new IntegrationOutbox();
      e.setEventId(UUID.randomUUID().toString());
      e.setTipo("ORDEN_ESTADO_CAMBIADO");
      e.setPayload(json.writeValueAsString(event));
      repo.save(e);
    } catch (Exception ex) {
      throw new IllegalStateException("No fue posible persistir evento de Orden", ex);
    }
  }
}