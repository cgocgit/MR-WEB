...
import mx.com.mesaregia.ordenes.security.internal.InternalJwtService;
...
class InternalJwtServiceTest {
  ...
  void emiteYValidaIdentidadInternaLocal() {
    var env = new MockEnvironment().withProperty("spring.application.name", "mr-ordenes-service");
    var jwt = new InternalJwtService(new ObjectMapper(), env);
    String token = jwt.issue();
    assertEquals("mr-ordenes-service", jwt.verifyIfInternal(token, Set.of("mr-ordenes-service")).orElseThrow());
  }
