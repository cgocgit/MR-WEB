package mx.com.mesaregia.pagos.security.internal;
...
public class InternalJwtService {
    ...
    public Optional<String> verifyIfInternal(String token, Set<String> allowedCallers) {
        try {
            ...
            if (!AUDIENCE.equals(String.valueOf(payload.get("aud")))) return Optional.empty();
            String issuer = String.valueOf(payload.get("iss"));
            if (!allowedCallers.contains(issuer)) throw new BadCredentialsException("Servicio interno no autorizado: " + issuer);
            String input = parts[0] + "." + parts[1];
            byte[] expected = hmac(input, secretFor(issuer));
            byte[] received = B64D.decode(parts[2]);
            if (!MessageDigest.isEqual(expected, received)) throw new BadCredentialsException("Firma de servicio interno inválida");
            long exp = ((Number) payload.get("exp")).longValue();
            if (exp < Instant.now().getEpochSecond()) throw new BadCredentialsException("Token interno expirado");
            return Optional.of(issuer);
        } catch (BadCredentialsException ex) { throw ex; }
        catch (Exception ex) { throw new BadCredentialsException("Token interno inválido", ex); }
    }
    ...
    private String secretFor(String service) {
        ...
        if (envName == null) throw new IllegalStateException("Servicio interno desconocido: " + service);
        String value = System.getenv(envName);
        if (value != null && !value.isBlank()) return value;
        boolean prod = Arrays.asList(environment.getActiveProfiles()).contains("prod");
        if (prod) throw new IllegalStateException("Falta variable obligatoria " + envName + " para identidad interna");
